...
public class SystemFinderImpl implements SystemFinder {
	...
	public void createElements() {
		...
		// Creacion Trip
		Long d1 = System.currentTimeMillis() + 24 * 60 * 60 * 1000;
		Trip trip1 = new Trip(new AddressPoint("direccion11", "ciudad11",
			...
			, new Date(d1 * 3), new Date(d1), new Date(d1 * 3)
			, 1, 2, 10.0,
				"comment1", TripStatus.OPEN, usuario1);
		...
		, new Date(d1 * 3), new Date(d1), new Date(d1 * 3)
		, 3, 4, 10.0,
		"comment1", TripStatus.OPEN, usuario3);
		...
		Seat seat13 = new Seat(usuario3,trip1);
		
		Seat seat21 = new Seat(usuario1,trip2);
		Seat seat22 = new Seat(usuario2,trip2);
		Seat seat23 = new Seat(usuario3,trip2);
		
		Seat seat31 = new Seat(usuario1,trip3);
		Seat seat32 = new Seat(usuario2,trip3);
		...
		Rating rating3 = new Rating(seat13, seat12, "ComentarioACompañero", 10);
		
		Factories.persistence.createRatingGateway().newRating(rating1);
		Factories.persistence.createRatingGateway().newRating(rating2);
		Factories.persistence.createRatingGateway().newRating(rating3);
	}
